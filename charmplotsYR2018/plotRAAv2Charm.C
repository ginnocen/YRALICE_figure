...
void plotRAAv2Charm()
{
    ...
    //// curves ////
    FILE *fDsTAMU = fopen("data_Ds_R_AA_010_TAMU.txt", "r"); //TAMU 0-10%
    TGraph *grDsTAMU1 = new TGraph(0);
    TGraph *grDsTAMU2 = new TGraph(0);

    Float_t pt, vmin, vmax;
    Int_t iPt = 0;
    Printf("---- TAMU -- ");
    while (!feof(fDsTAMU))
    {
        fscanf(fDsTAMU, "%f %f %f\n", &pt, &vmin, &vmax);
        grDsTAMU1->SetPoint(iPt, pt, vmin);
        grDsTAMU2->SetPoint(iPt, pt, vmax);
        Printf("%f %f %f", pt, vmin, vmax);
        iPt++;
    }

    grDsTAMU1->SetLineColor(kRed - 3);
    grDsTAMU1->SetLineWidth(3);
    grDsTAMU1->SetLineStyle(7);
    grDsTAMU2->SetLineColor(kRed - 3);
    grDsTAMU2->SetLineWidth(3);
    grDsTAMU2->SetLineStyle(7);

    FILE *fDsPHSD = fopen("phsd-alice-1.txt", "r"); //PHSD 0-10%
    TGraph *grDsPHSD = new TGraph(0);

    Printf("---- PHSD -- ");
    Float_t RaaD010, RaaD6080, RaaDs010, RaaDs6080;
    iPt = 0;
    while (!feof(fDsPHSD))
    {
        fscanf(fDsPHSD, "%f %f %f %f %f\n", &pt, &RaaD010, &RaaD6080, &RaaDs010, &RaaDs6080);
        grDsPHSD->SetPoint(iPt++, pt, RaaDs010);
        Printf("%f %f", pt, RaaD010);
    }
    grDsPHSD->SetLineColor(kRed - 3);
    // grDsPHSD->SetLineColor(kGreen + 4);
    grDsPHSD->SetLineWidth(4);
    //grDsPHSD->SetLineStyle(2);
    ...
    eyh[10] = 0.0291031;
    //print on file
    FILE *fDout = fopen("fALICE_CharmRAAv2.txt", "w");
    //pt,RAA,ex,stat,syst
    fprintf(fDout, "D0 (0-10%% centr., Lint=10nb-1): pt, width x, RAA (BAMPS), err stat, err syst low, err syst high \n \n \n");
    printf("D0 (0-10%% centr., Lint=10nb-1): pt, width x, RAA, err stat, err syst low, err syst high \n \n \n");
    ...
    TLatex *texD0 = new TLatex(4, 1.8, "0-10% Pb-Pb, #sqrt{s_{NN}}=5.5 TeV, #it{L}_{int} = 10 nb^{-1}");
    ...
    for (Int_t ii = 0; ii < 11; ii++)
    {
        ...
        Double_t relerrRAA = hRaaDs->GetBinError(ii + 1) / hRaaDs->GetBinContent(ii + 1);

        for (Int_t kk = 0; kk < 30; kk++)
        { //scale Ds RAA (and error) to TAMU highest curve
            Double_t x, y;
            grDsTAMU1->GetPoint(kk, x, y);
            if (xs[ii] >= (x - 0.5) && xs[ii] < (x + 0.5))
            {
                cout << "HERE ---> " << xs[ii] << "  " << x << endl;
                hRaaDs->SetBinContent(ii + 1, y);
                hRaaDs->SetBinError(ii + 1, y * relerrRAA);
            }
        }
    ...
    TLatex *texDs0 = new TLatex(4, 1.8, "ALICE Upgrade Simulation");
    texDs0->SetTextFont(42);
    texDs0->SetLineWidth(2);
    texDs0->SetTextSize(0.045);
    TLatex *texDs = new TLatex(4, 1.6, "0-10% Pb-Pb, #sqrt{s_{NN}}=5.5 TeV, #it{L}_{int} = 10 nb^{-1}");
    texDs->SetTextFont(42);
    texDs->SetTextSize(0.045);
    TLatex *texDs2 = new TLatex(4, 1.6, "D_{s}^{+}#rightarrow K^{+} K^{-} #pi^{+}");
    texDs2->SetTextFont(42);
    texDs2->SetTextSize(0.04);
    TLegend *legRAA = new TLegend(0.5, 0.5, 0.56, 0.66);
    legRAA->AddEntry(hRaaPrompt, "D^{0}", "p");
    legRAA->AddEntry(hRaaDs, "D^{+}_{s}", "p");
    legRAA->SetBorderSize(0);

    hRaaPrompt->GetYaxis()->SetTitle("#it{R}_{AA}");
    hRaaPrompt->GetXaxis()->SetTitle("#it{p}_{T} (GeV/#it{c})");
    TCanvas *cans = new TCanvas("cans", "cans", 800, 650);
    ...
    //texDs2->Draw();
    legRAA->Draw();
    cans->SaveAs("ALICEUpgrade_charmRAA.pdf");
    fprintf(fDout, "Ds (0-10%% centr., Lint=10nb-1): pt, width x, RAA (TAMU), err stat, err syst low, err syst high \n \n \n");
    printf("Ds (0-10%% centr., Lint=10nb-1): pt, width x, RAA, err stat, err syst low, err syst high \n \n \n");
    for (Int_t ii = 0; ii < 11; ii++)
    {
    ...
    TLegend *legRAADs = new TLegend(0.45, 0.45, 0.8, 0.65);
    legRAADs->AddEntry(hRaaPrompt, "D^{0}", "p");
    legRAADs->AddEntry(hRaaDs, "D^{+}_{s}", "p");

    legRAADs->AddEntry(grDsPHSD, "PHSD, D^{+}_{s}", "l");
    legRAADs->AddEntry(grDsTAMU1, "TAMU, D^{+}_{s}", "l");
    legRAADs->SetBorderSize(0);

    TCanvas *cansTheo = new TCanvas("cansTheo", "cansTheo", 800, 650);
    cansTheo->SetLeftMargin(0.13);
    cansTheo->SetBottomMargin(0.13);
    cansTheo->SetTickx();
    cansTheo->SetTicky();
    hRaaPrompt->Draw();
    hRaaDs->Draw("same");
    ...
    grD0Raasyst->Draw("2same");
    grDsPHSD->Draw("lsame");
    grDsTAMU1->Draw("lsame");
    grDsTAMU2->Draw("lsame");
    ...
    texDs->Draw();
    legRAADs->Draw();
    cansTheo->SaveAs("ALICEUpgrade_charmRAATheoDs.pdf");
    ...
    grLcOvD0stat->GetXaxis()->SetTitle("#it{p}_{T} (GeV/c)");
    ...
    fprintf(fDout, "D0 (30-50%% centr., Lint=10nb-1): pt, width x, v2 (BAMPS), err stat, err syst \n \n \n");
    printf("D0 (30-50%% centr., Lint=10nb-1): pt, width x, v2, err stat, err syst \n \n \n");
    ...
    for (Int_t ii = 0; ii < 31; ii++)
    {
        ...
        {
            erryield = yierr[0];
            Double_t erryieldonv2Formula = TMath::Pi() / 4. / TMath::Sqrt(2.) * (1. - 4 * yv2[ii] / TMath::Pi() * 4 * yv2[ii] / TMath::Pi()) * erryield;

            Double_t erryieldonv2 = 0.02; //instead of formula, based on Run 2 AN
            errfd = fderr[0];
            ...
            cout << "***** erryieldonv2 =" << erryieldonv2 << " erryieldonv2 Formula =" << erryieldonv2Formula << endl;
        }
        else
        {
            erryield = yierr[1];
        ...
        // cout
        //   << "D0  --> " << ii << "  " << xv2[ii] << "  " << yv2[ii] << "  " << exsyst[ii] << "  relerr=" << relerr << "  uncv2=" << uncv2[ii] << endl;
    ...
    hEllipticFlowDs->SetMaximum(0.3);
    hEllipticFlowDs->GetXaxis()->SetRangeUser(0, 16);
    hEllipticFlowDs->GetXaxis()->SetTitle("#it{p}_{T} (GeV/#it{c})");
    hEllipticFlowDs->GetYaxis()->SetTitle("#it{v}_{2}");
    ...
    fprintf(fDout, "Ds (30-50%% centr., Lint=10nb-1): pt, width x, v2 (TAMU), err stat, err syst \n \n \n");
    printf("Ds (30-50%% centr., Lint=10nb-1): pt, width x, v2, err stat, err syst \n \n \n");
    ...
    for (Int_t ii = 0; ii < nDs; ii++)
    {
        ...
        uncv2Ds[ii] = TMath::Pi() / 4. / TMath::Sqrt(2.) * (1. - 4 * yDs[ii] / TMath::Pi() * 4 * yDs[ii] / TMath::Pi()) * relerrDs;

        // cout << "Ds  --> " << ii << "  " << xDs[ii] << "  " << yDs[ii] << "  relerrDs=" << relerrDs << "  uncv2Ds=" << uncv2Ds[ii] << endl;
    ...
    printf("Lambdac (10-40%% centr., Lint=10nb-1): pt, width x, v2, err stat, err syst \n \n \n");

    for (Int_t ii = 0; ii < 3; ii++)
    {
        exsystLc[ii] = 0.1;
        Double_t erryieldonv2Formula = TMath::Pi() / 4. / TMath::Sqrt(2.) * (1. - 4 * yLc[ii] / TMath::Pi() * 4 * yLc[ii] / TMath::Pi()) * erryieldLc;
        Double_t erryieldonv2 = 0.02;
        cout << "Lc: erryieldonv2=" << erryieldonv2 << "  erryieldonv2Formula=" << erryieldonv2Formula << endl;
        uncv2Lc[ii] = TMath::Sqrt(erryieldonv2 * erryieldonv2 + errfdLc * yLc[ii] * errfdLc * yLc[ii]);
    ...
    TLatex *tex0 = new TLatex(3., 0.27, "ALICE Upgrade Simulation");
    tex0->SetTextFont(42);
    tex0->SetLineWidth(2);
    tex0->SetTextSize(0.045);
    TLatex *tex = new TLatex(3., 0.24, "Pb-Pb, #sqrt{s_{NN}}=5.5 TeV, #it{L}_{int} = 10 nb^{-1}");
    tex->SetTextFont(42);
    tex->SetLineWidth(2);
    ...
    leg->SetLineWidth(2);
    leg->SetBorderSize(0);
    TCanvas *canv2 = new TCanvas("canv2", "canv2", 800, 650);
    canv2->SetTickx();
    canv2->SetTicky();
    ...
    tex0->Draw();
    leg->Draw();
    canv2->SaveAs("ALICEUpgrade_charmv2.pdf");
}